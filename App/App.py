...
def add_course():
    if request.method == 'POST':
        ...
        if existing_course:
            message = 'Course with the same code already exists. Please choose a different code.'
            cursor.execute("SELECT * FROM college")
            colleges = cursor.fetchall()
            return render_template('add_course.html', colleges=colleges, message=message)
...
def add_college():
    if request.method == 'POST':
        ...
        if existing_college:
            message = 'College with the same code already exists. Please choose a different code.'
            return render_template('add_college.html', message=message)
